 ...
 * Pass `jitter: 0` (or inject) for deterministic tests. Cap 30s.
 */
export function toBackoffMs(attempt, {base = 700, cap = 30_000, jitter = 0.2, random = Math.random} = {}) {
    const exp = Math.min(cap, base * (2 ** Math.max(0, attempt - 1)));
    if (!jitter || jitter <= 0) return exp;
    const spread = exp * Math.min(1, jitter);
    return Math.max(0, Math.floor(exp - spread + random() * spread * 2));
}
