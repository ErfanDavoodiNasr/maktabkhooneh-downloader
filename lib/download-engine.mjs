...
export async function probeRemoteSize(url, {
    ...
    try {
        const res = await fetchFn(url, {method: 'HEAD', headers, redirect: 'manual'}, requestTimeoutMs);
...
export async function downloadToFile(url, filePath, {
    ...
    if (sampleBytes === 0 && existingFinalSize > 0) {
        remoteInfo = await probeRemoteSize(url, {
            cookie, referer, fetchFn, requestTimeoutMs, trustedOrigin, allowPrivateMedia
        });
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
            ...
            const res = await fetchFn(url, {method: 'GET', headers, redirect: 'follow'}, requestTimeoutMs);
