...
- پیش‌نمایش قبل از دانلود (Dry Run): نمایش لیست فایل‌ها + حجم تقریبی هر قسمت/فصل + جمع کل
...
- خطاهای قابل‌اقدام (Actionable Errors): علت خطا + دستور بعدی پیشنهادی
...
پیش‌نمایش قبل از دانلود (بدون دانلود واقعی فایل‌ها):
```powershell
...
## 👀 پیش‌نمایش قبل از دانلود (Dry Run)
اگر می‌خواهید قبل از مصرف اینترنت، برآورد خوبی از حجم دانلود داشته باشید:
...
در این حالت:
- هیچ فایلی دانلود نمی‌شود.
- پوشه/فایل خروجی ساخته نمی‌شود.
- برای هر قسمت، موارد زیر نمایش داده می‌شود:
  - حجم تقریبی ویدیو
  - تعداد و حجم تقریبی زیرنویس‌ها
  - تعداد و حجم تقریبی ضمیمه‌ها
  - مسیر خروجی فایل
- برای هر فصل، خلاصه‌ی حجم تقریبی نمایش داده می‌شود.
- در انتها، جمع کل حجم تقریبی کل دوره نمایش داده می‌شود.

نکته مهم:
- اندازه‌ها «تخمینی» هستند و بر اساس اطلاعاتی که سرور از طریق `HEAD/Range` می‌دهد محاسبه می‌شوند.
- اگر برای بعضی آیتم‌ها اندازه قابل تشخیص نباشد، به صورت `unknown` گزارش می‌شوند.
...
# پیش‌نمایش فقط برای فصل 2 و قسمت‌های 2 تا 5 و 9
